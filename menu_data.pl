...
if( $ui eq 'win32' ) {
  ...
  die "$0: dump_qt: which mode -- cpp, or ui?" unless @ARGV >= 1;
...
void DeFuseWindow::menu_data_init() {
...
sub _dump_qt ($$$$$) {
  ...
  my( $mode, $menu, $prefix, $path, $spaces ) = @_;
  ...
  foreach my $item ( @{ $menu->{submenu} } ) {
    ...
    if( $mode eq 'cpp' ) {
      if( $item->{submenu} ) {
        _dump_qt( $mode, $item, "${prefix}${cname}_", "${path}_$cname", $spaces . "  " );
      } else {
	      my $function = $item->{function} || "${path}_$cname";
        my $action = $item->{action} || 0;
        print << "CODE";
  QObject::connect(ui->action_${prefix}${cname}, &QAction::triggered, this, [&]() { $function( ${action} ); });
CODE
      }
...
XML
      ...
      if( $item->{submenu} ) {
            print << "XML";
${spaces}<widget class="QMenu" name="menu_${prefix}${cname}">
...
XML
        ...
        print "${spaces}</widget>\n";
        print "${spaces}<addaction name=\"menu_${prefix}${cname}\"/>\n";
      } else {
          print "${spaces}<addaction name=\"action_${prefix}${cname}\"/>\n";
      }
    } 
  }
}
